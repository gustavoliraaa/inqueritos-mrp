...
import { Building2, BriefcaseBusiness, CircleHelp, Flag, LoaderCircle, Network, Plus, Search, UserRound, X } from "lucide-react";
...
type Organization = {
  ...
  identifier: string;
  name: string;
  data: { type?: string; locations?: string; person_id?: string; person_name?: string; notes?: string };
  updated_at: string;
};
...
const organizationTypes = [
  { value: "Facção", description: "Grupo organizado", tone: "red", icon: Flag },
  { value: "Gangue", description: "Atuação territorial", tone: "purple", icon: Network },
  { value: "Empresa", description: "Atividade comercial", tone: "blue", icon: BriefcaseBusiness },
  { value: "Grupo", description: "Organização informal", tone: "green", icon: Building2 },
  { value: "Outro", description: "Outro tipo de vínculo", tone: "amber", icon: CircleHelp }
] as const;

const emptyForm = { identifier: "", name: "", type: "Facção", locations: "", person_id: "", notes: "" };
...
export default function OrganizationsPage() {
  ...
  async function saveOrganization(event: FormEvent<HTMLFormElement>) {
    ...
    const data = { type: form.type, locations: form.locations.trim(), person_id: form.person_id || undefined, person_name: selectedPerson?.name, notes: form.notes.trim() };
    let operationError = "";
    if (editing) {
  ...
  const filtered = organizations.filter((organization) => `${organization.identifier} ${organization.name} ${organization.data?.type || ""} ${organization.data?.locations || ""}`.toLowerCase().includes(query.toLowerCase()));
