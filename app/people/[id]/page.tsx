      ...
      <section className="content">
        <header className="topbar">
          <div className="breadcrumbs">
            <button className="breadcrumb-link" onClick={() => router.push("/")}>Central</button>
            <span>/</span>
            <button className="breadcrumb-link" onClick={() => router.push("/people")}>Pessoas</button>
            <span>/</span>
        ...
        <div className="page person-detail-page">
          <div className="page-heading">
            <div><p className="eyebrow">{person.identifier}</p><h1>{person.name}</h1><p className="muted">Visão consolidada do cadastro central.</p></div>
            <button className="secondary-button" onClick={() => router.push("/people")}><ArrowLeft size={16} /> Voltar</button>
          </div>
          <div className="person-summary-grid">
            <section className="panel person-summary-card">
              <div className="person-large-avatar">{person.name.split(" ").map((part) => part[0]).slice(0, 2).join("").toUpperCase()}</div>
              <h2>{person.name}</h2>
              <span className="person-status">{person.status === "active" ? "Cadastro ativo" : "Cadastro inativo"}</span>
              <button className="secondary-button edit-person-button" onClick={() => router.push("/people")}>Voltar para cadastros</button>
            </section>
            <section className="panel person-data-card">
              <div className="panel-heading"><div><h2>Dados cadastrais</h2><p>Informações registradas na base central.</p></div></div>
              <dl className="person-data-list">
                <div><dt>Identificador</dt><dd>{person.identifier}</dd></div>
                <div><dt>Documento fictício</dt><dd>{person.document_id || "Não informado"}</dd></div>
                <div><dt>Data de nascimento</dt><dd>{person.birth_date ? new Date(`${person.birth_date}T00:00:00`).toLocaleDateString("pt-BR") : "Não informada"}</dd></div>
                <div><dt>Apelidos</dt><dd>{person.aliases.length ? person.aliases.join(", ") : "Nenhum registrado"}</dd></div>
                <div className="full-width"><dt>Observações</dt><dd>{person.notes || "Nenhuma observação registrada."}</dd></div>
              </dl>
            </section>
          </div>
