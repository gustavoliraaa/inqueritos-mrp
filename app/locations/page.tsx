...
import { Building2, Home, LoaderCircle, Plus, Search, X } from "lucide-react";
...
type Location = {
  ...
  identifier: string;
  name: string;
  data: { organization_id?: string; organization_name?: string; organization_type?: string; address?: string; neighborhood?: string; owner?: string; notes?: string };
  updated_at: string;
};

type OrganizationOption = { id: string; identifier: string; name: string; data: { type?: string } };

const emptyForm = { identifier: "", name: "", organization_id: "", address: "", neighborhood: "", owner: "", notes: "" };
...
export default function LocationsPage() {
  ...
  async function loadLocations() {
    ...
    const [locationsResult, organizationsResult] = await Promise.all([
      ...
      supabase
        .from("entities")
        .select("id, identifier, name, data")
        .eq("entity_type", "organization")
        .order("name", { ascending: true })
    ]);
    if (locationsResult.error) setError("Não foi possível carregar os locais.");
    else setLocations((locationsResult.data || []) as Location[]);
    if (organizationsResult.error) setError("Não foi possível carregar as organizações para o cadastro de locais.");
    else setOrganizations((organizationsResult.data || []) as OrganizationOption[]);
    setLoading(false);
  }
  ...
  function openEdit(location: Location) {
    ...
    setForm({
      ...
      name: location.name,
      organization_id: location.data?.organization_id || "",
  ...
  async function saveLocation(event: FormEvent<HTMLFormElement>) {
    ...
    const organization = organizations.find((item) => item.id === form.organization_id);
    if (!organization) {
      setError("Selecione uma organização cadastrada para vincular o local.");
      setSaving(false);
      return;
    }
    const data = {
      ...
      organization_type: organization.data?.type || "Não informado",
  ...
  const filtered = locations.filter((location) =>
    `${location.identifier} ${location.name} ${location.data?.organization_name || ""} ${location.data?.organization_type || ""} ${location.data?.address || ""} ${location.data?.neighborhood || ""} ${location.data?.owner || ""}`
      .toLowerCase()
