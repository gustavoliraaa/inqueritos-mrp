...
import { Archive, Box, CheckCircle2, CircleHelp, FileText, FileUp, Image, LoaderCircle, Mic, Plus, Search, Shield, Video, X } from "lucide-react";
...
const evidenceTypes = [
  { value: "Documento", description: "Arquivos e registros", tone: "blue", icon: FileText },
  { value: "Imagem", description: "Fotos e capturas", tone: "purple", icon: Image },
  { value: "Vídeo", description: "Gravações audiovisuais", tone: "red", icon: Video },
  { value: "Áudio", description: "Gravações sonoras", tone: "amber", icon: Mic },
  { value: "Objeto", description: "Item físico apreendido", tone: "green", icon: Box },
  { value: "Relatório", description: "Análise ou documento técnico", tone: "blue", icon: FileText },
  { value: "Outro", description: "Outro tipo de evidência", tone: "purple", icon: CircleHelp }
] as const;
const evidenceStatuses = [
  { value: "active", label: "Ativa", description: "Em acompanhamento", tone: "green", icon: CheckCircle2 },
  { value: "analyzed", label: "Analisada", description: "Revisão concluída", tone: "blue", icon: CheckCircle2 },
  { value: "archived", label: "Arquivada", description: "Fora de uso ativo", tone: "amber", icon: Archive }
] as const;
...
export default function EvidencesPage() {
  ...
  const filteredInvestigations = investigations.filter((investigation) =>
    `${investigation.identifier} ${investigation.title}`.toLowerCase().includes(investigationSearch.toLowerCase())
  );
  ...
  return (
    <main className="shell">
      ...
      {showForm && <div className="modal-backdrop" onClick={() => setShowForm(false)}><form className="modal person-modal" onSubmit={saveEvidence} onClick={(event) => event.stopPropagation()}><button type="button" className="modal-close" onClick={() => setShowForm(false)}><X size={18} /></button><p className="eyebrow">{editing ? editing.identifier : "NOVO REGISTRO"}</p><h2>{editing ? "Editar evidência" : "Cadastrar evidência"}</h2><label>Identificador fictício <span className="field-hint">(opcional)</span><input value={form.identifier} placeholder="Ex.: EVD-000001" onChange={(event) => setForm({ ...form, identifier: event.target.value })} /></label><label className="investigation-selector-label">Inquérito relacionado<div className="investigation-search"><Search size={15} /><input aria-label="Buscar inquérito" placeholder="Buscar por número ou título..." value={investigationSearch} onChange={(event) => setInvestigationSearch(event.target.value)} /></div><select value={form.investigation_id} onChange={(event) => setForm({ ...form, investigation_id: event.target.value })}><option value="">Não vincular agora</option>{filteredInvestigations.map((investigation) => <option key={investigation.id} value={investigation.id}>{investigation.identifier} · {investigation.title}</option>)}</select>{investigations.length > 0 && filteredInvestigations.length === 0 && <small>Nenhum inquérito corresponde à busca.</small>}</label><label>Título<input required minLength={3} value={form.title} onChange={(event) => setForm({ ...form, title: event.target.value })} /></label><fieldset className="role-field"><legend>Tipo de evidência</legend><div className="role-selector" role="radiogroup" aria-label="Tipo de evidência">{evidenceTypes.map((option) => { const Icon = option.icon; return <button className={`role-option ${option.tone} ${form.evidence_type === option.value ? "selected" : ""}`} key={option.value} type="button" role="radio" aria-checked={form.evidence_type === option.value} onClick={() => setForm({ ...form, evidence_type: option.value })}><span className="role-option-icon"><Icon size={15} /></span><span className="role-option-copy"><strong>{option.value}</strong><small>{option.description}</small></span><span className="role-option-check" aria-hidden="true">{form.evidence_type === option.value ? "✓" : ""}</span></button>; })}</div></fieldset><label>Descrição<textarea value={form.description} onChange={(event) => setForm({ ...form, description: event.target.value })} /></label><label>Data da coleta<input type="date" value={form.collected_at} onChange={(event) => setForm({ ...form, collected_at: event.target.value })} /></label><label>Local da coleta<input value={form.collected_location} onChange={(event) => setForm({ ...form, collected_location: event.target.value })} /></label><fieldset className="role-field"><legend>Status da evidência</legend><div className="role-selector" role="radiogroup" aria-label="Status da evidência">{evidenceStatuses.map((option) => { const Icon = option.icon; return <button className={`role-option ${option.tone} ${form.status === option.value ? "selected" : ""}`} key={option.value} type="button" role="radio" aria-checked={form.status === option.value} onClick={() => setForm({ ...form, status: option.value })}><span className="role-option-icon"><Icon size={15} /></span><span className="role-option-copy"><strong>{option.label}</strong><small>{option.description}</small></span><span className="role-option-check" aria-hidden="true">{form.status === option.value ? "✓" : ""}</span></button>; })}</div></fieldset><label>Anexo <span className="field-hint">(opcional, até 10 MB)</span><input type="file" accept="image/*,.pdf,.doc,.docx,.xls,.xlsx,.txt,.mp3,.wav,.mp4" onChange={(event) => setAttachment(event.target.files?.[0] || null)} />{editing?.storage_path && <span className="field-hint">Já existe um anexo. Selecione outro para substituí-lo.</span>}</label>{error && <p className="form-error">{error}</p>}<div className="modal-actions"><button type="button" className="secondary-button" onClick={() => setShowForm(false)}>Cancelar</button><button className="primary-button" disabled={saving}>{saving ? "Salvando..." : "Salvar evidência"}</button></div></form></div>}
    </main>
  );
}
