...
import { Archive, ArrowLeft, LoaderCircle, Plus, Search, Shield, X } from "lucide-react";
...
export default function EvidencesPage() {
  ...
  async function loadData() {
    ...
    const [{ data: evidenceData, error: evidenceError }, { data: investigationData, error: investigationError }] = await Promise.all([
      supabase.from("evidences").select("id, identifier, investigation_id, evidence_type, title, description, collected_at, collected_location, status, created_at, investigations(identifier, title)").order("created_at", { ascending: false }),
  ...
  const filtered = evidences.filter((evidence) => `${evidence.identifier} ${evidence.title} ${evidence.evidence_type} ${evidence.investigations?.identifier || ""} ${evidence.investigations?.title || ""} ${evidence.collected_location || ""}`.toLowerCase().includes(query.toLowerCase()));
